...
export const attackByFunc = ({inventory}:GameState) => (attacker: Mortal, defender: Mortal): AttackResult => {

  const magicBonus = inventory?.reduce((bonus, item) => isMagic(item) ? bonus + 1 : bonus, 0) ?? 0
  const attackBonus = isPlayer(attacker) ? inventory?.reduce((bonus, item) => isWeapon(item) ? bonus + 1 : bonus, 0) ?? 0 + magicBonus : 0
  const defenseBonus = isPlayer(defender) ? inventory?.reduce((bonus, item) => isArmor(item) ? bonus + 1 : bonus, 0) ?? 0 + magicBonus : 0
