  ...
  switch (gameState.action) {
    ...
    case "search": {
      ...
      if (secret) {
        ...
        return { ...gameState, doors, message: `You discover a secret door to the ${secret.towards}!` }
      }
...
const describeRoomFunc = (getCurrentRoom: (id: number) => Room) => (gameState: GameState): { description: string, exits: Exit[] } => {
  ...
  const isVisible = isVisibleExitFunc(gameState)
  const exits = room.exits.filter(isVisible).slice(0).sort((a, b) => b.door.id - a.door.id)
...
export const game = (dungeon: Dungeon): GameInterface => {
  ...
  const gameInterface = (input: string) => {
    if (gameState.turn > 0) {
      ...
      return { message: gameState.message, room: gameState.id, ...describeRoom(gameState), end: gameState.end, error: gameState.error, action }
    }
    else {
