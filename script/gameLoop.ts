        ...
        default:
          if (/\d/.test(gameState.action)) {
            const exit = currentRoom.exits
              .filter(isVisible)
              .slice(0)
              .sort(sortExitsClockwise(currentRoom))[parseInt(gameState.action) - 1]
            return handleExit(exit, dungeon, gameState)
...
const sortExitsClockwise = (room: Room) => (aExit: Exit, bExit: Exit): 1 | 0 | -1 => {
  const a = aExit.door
  const b = bExit.door
...
const describeRoomFunc =
  (getCurrentRoom: (id: number) => Room) =>
    (gameState: GameState): { description: string; exits: Exit[] } => {
      ...
      const isVisible = isVisibleExitFunc(gameState)
      const areExitsSame = room.exits.some((exit, i, all) => [...all.slice(0, i), ...all.slice(i + 1)].find((e) => e.towards === exit.towards))
      ...
      const exits = room.exits
        ...
        .map((exit, i) => ({ ...exit, description: `To the ${exit.towards} ${isA(exit.description)} ${exit.description}${exitNumber(areExitsSame, i)}` }))

      // areExitsSame is true if there are 2 or more exits with the same direction
      const description =
        `You are in a ${room.area} ${room.description} ${room.contains ? hereIs(room.contains) : ""} `
