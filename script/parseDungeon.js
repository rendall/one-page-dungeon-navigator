...
var describeRoom = function (room, exits, columns, water) {
    ...
    var columnDesc = columns && columns.length > 0
        ? room.rotunda
        ...
        : "";
    var waterDesc = water && water.length > 0
...
var getRoomNoun = function (room, exits) {
    if (room === "outside")
        return room;
    var exitsLength = exits.filter(function (exit) { return exit.description !== "secret door"; }).length;
    if (is1x1(room)) {
        switch (exitsLength) {
            case 1:
                return "alcove";
            case 2:
                var _a = exits.map(function (exit) { return exit.towards; }), dir1 = _a[0], dir2 = _a[1];
...
var is1x1 = function (a) { return a.w === 1 && a.h === 1; };
var isAdjacent = function (a, b) {
    if (!is1x1(a) && !is1x1(b))
        return false; // in this format, if rects are adjacent, one of them must by 1 x 1
    if (!is1x1(b))
        return isAdjacent(b, a); // makes things easier if the 2nd is always the 1 x 1
    var left = a.x;
    var top = a.y;
    var right = a.x + a.w;
    var bottom = a.y + a.h;
    ...
    if (isTop || isBottom)
        return b.x >= a.x && b.x < right;
    if (isRight || isLeft)
        return b.y >= a.y && b.y < bottom;
    return false;
};
...
var doorFunc = function (doors) { return function (a) {
    return doors.find(function (door) { return door.x === a.x && door.y === a.y; });
}; };
var getAdjacent = function (a, rects) {
    return rects.filter(function (rect) { return isAdjacent(rect, a); });
};
    ...
    var rooms = rectsWithId
        ...
        var exits = getAdjacent(fullRoom, rectsWithId).map(function (exit) {
            ...
            if (door) {
                ...
                return { towards: direction, to: exit.id };
        });
        var door = getDoor(fullRoom);
        var contains = (_b = (_a = notes.filter(function (note) { return isInside(note.pos, fullRoom); })) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.text;
        ...
        var room = __assign(__assign(__assign(__assign({ id: fullRoom.id, description: description, area: fullRoom.rotunda
